     ...
     async payOrder() {
       ...
       // const { data: res } = await uni.$http.post('/api/public/v1/my/orders/create', orderInfo)
        const res={
          "message": {
            "order_id": 45,
            "user_id": 12,
            "order_number": "HMDD20200901000000001949",
            "order_price": 0.1,
            "order_pay": "0",
            "is_send": "否",
            "trade_no": "",
       ...
       console.log(orderNumber)
        ...
        // 3. 发起微信支付
           ...
           // const { data: res3 } = await uni.$http.post('/api/public/v1/my/orders/chkOrder', { order_number: res2.message.orderNumber })
           // 3.4 检测到订单未支付
         ...
         // 2. 判断秒数是否 <= 0
             if (this.seconds <= 0) {
               ...
               uni.switchTab({
                 ...
                 // 页面跳转成功之后的回调函数
                         success: () => {
                           // 调用 vuex 的 updateRedirectInfo 方法，把跳转信息存储到Store中的 redirectInfo对象中
